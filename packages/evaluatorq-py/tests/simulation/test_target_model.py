...
def _make_mock_judgment(
	*, should_terminate: bool = True, goal_achieved: bool = True
) -> MagicMock:
	j = MagicMock()
...
class TestTargetModelCallback:
	...
	@pytest.mark.asyncio
	async def test_callback_target_does_not_set_target_model(
	...
	@pytest.mark.asyncio
	async def test_callback_target_target_model_is_absent_or_none(
		self, monkeypatch: pytest.MonkeyPatch
	) -> None:
...
class TestTargetModelAgentTarget:
	...
	@pytest.mark.asyncio
	async def test_agent_target_with_model_surfaces_target_model(
		self, monkeypatch: pytest.MonkeyPatch
	) -> None:
		...
		assert result.metadata.get('target_model') == expected_model, (
			f'Expected target_model="{expected_model}", '
			f'got {result.metadata.get("target_model")!r}'
		)

	@pytest.mark.asyncio
	async def test_agent_target_without_model_leaves_target_model_absent(
		self, monkeypatch: pytest.MonkeyPatch
	) -> None:
	...
	@pytest.mark.asyncio
	async def test_target_model_consistent_across_termination_modes(
		self, monkeypatch: pytest.MonkeyPatch
	) -> None:
		...
		assert result.metadata.get('target_model') == expected_model, (
			f'Expected target_model="{expected_model}" on max_turns path, '
			f'got {result.metadata.get("target_model")!r}'
		)
