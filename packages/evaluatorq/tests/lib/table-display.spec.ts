...
describe("calculateEvaluatorAverages", () => {
  test("calculates average for number scores", () => {
    const results: EvaluatorqResult = [
      makeResult("job1", [{ evaluatorName: "accuracy", value: 0.5 }]),
      makeResult("job1", [{ evaluatorName: "accuracy", value: 1.0 }]),
    ];
  ...
  test("calculates pass rate for boolean scores", () => {
    const results: EvaluatorqResult = [
      makeResult("job1", [{ evaluatorName: "pass_check", value: true }]),
      makeResult("job1", [{ evaluatorName: "pass_check", value: false }]),
    ];
  ...
  test("renders string scores as [string]", () => {
    const results: EvaluatorqResult = [
      makeResult("job1", [{ evaluatorName: "quality", value: "good" }]),
    ];
  ...
  test("handles empty results without crashing", () => {
    const { jobNames, evaluatorNames, averages } = calculateEvaluatorAverages(
      [],
    );
  ...
  test("100% boolean pass rate uses correct value", () => {
    const results: EvaluatorqResult = [
      makeResult("job1", [{ evaluatorName: "check", value: true }]),
      makeResult("job1", [{ evaluatorName: "check", value: true }]),
    ];
