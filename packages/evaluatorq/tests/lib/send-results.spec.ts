...
describe("sendResultsToOrqEffect serialization", () => {
	...
	function buildResults(
		scoreValue: number | boolean | string | { type: string; value: Record<string, unknown> },
		error?: Error,
	): EvaluatorqResult {
		return [
			{
	...
	function capturePayload(results: EvaluatorqResult): Promise<Record<string, unknown>> {
		return new Promise((resolve) => {
			...
			Effect.runPromise(
				sendResultsToOrqEffect(apiKey, evalName, undefined, undefined, results, startTime, endTime),
			).finally(() => {
	...
	function extractFirstEvalScore(payload: Record<string, unknown>): Record<string, unknown> {
		const results = payload.results as Array<Record<string, unknown>>;
		const jobResults = results[0].jobResults as Array<Record<string, unknown>>;
		const evalScores = jobResults[0].evaluatorScores as Array<Record<string, unknown>>;
		return evalScores[0];
	}

	test("serializes number score value as-is", async () => {
		const payload = await capturePayload(buildResults(0.85));
		const score = extractFirstEvalScore(payload).score as Record<string, unknown>;
	...
	test("serializes Error objects to strings", async () => {
		const payload = await capturePayload(buildResults(0.5, new Error("eval failed")));
