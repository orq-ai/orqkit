...
// Example using a dataset from Orq platform
export async function runDatasetExample() {
  console.log("Running evaluation with dataset from Orq platform...\n");

  const result = await evaluatorq("dataset-evaluation", {
    data: {
  ...
  console.log(`Total data points processed: ${result.length}`);

  // Calculate summary statistics
  const successfulJobs = result.filter((r) => !r.error && r.jobResults).length;
  const failedJobs = result.filter((r) => r.error).length;

  console.log(`Successful: ${successfulJobs}, Failed: ${failedJobs}`);
